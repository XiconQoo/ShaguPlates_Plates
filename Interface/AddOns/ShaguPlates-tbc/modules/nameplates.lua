ShaguPlates:RegisterModule("nameplates", "vanilla:tbc", function ()
  ...
  nameplates.OnCreate = function(frame)
    ...
    nameplate.totem:SetHeight(27)
    nameplate.totem:SetWidth(27)
    ...
    if parent:GetScript('OnShow') then
      parent:HookScript('OnShow', nameplates.OnShow)
    else
      parent:SetScript('OnShow', nameplates.OnShow)
    end

    if parent:GetScript('OnUpdate') then
      parent:HookScript('OnUpdate', nameplates.OnUpdate)
    else
  ...
  nameplates.OnDataChanged = function(self, plate)
    ...
    if (target or mouseover) and C.nameplates.targetglow == "1" then
      plate.glow:Show() else plate.glow:Hide()
    end

    -- target indicator
    if (target or mouseover) and C.nameplates.targethighlight == "1" then
  ...
  nameplates.OnUpdate = function(frame)
    ...
    local level = nameplate.original.level:IsShown() and nameplate.original.level:GetObjectType() == "FontString" and tonumber(nameplate.original.level:GetText()) or "??"
    ...
    local hp = nameplate.original.healthbar:GetValue()
    local hpmin, hpmax = nameplate.original.healthbar:GetMinMaxValues()

    nameplate.name:SetText(name)
    nameplate.level:SetText(string.format("%s%s", level, (elitestrings[elite] or "")))
    ...
    if C.nameplates.showhp == "1" then
      ...
      if C.nameplates.alwaysperc == "0" and ( estimated or hpmax > 100 or (round(hpmax/100*hp) ~= hp) ) then
        nameplate.health.text:SetText(string.format("%s / %s", Abbreviate(rhp), Abbreviate(rhpmax)))
      else
        nameplate.health.text:SetText(string.format("%s%%", ceil(hp/hpmax*100)))
      end
    else
      nameplate.health.text:SetText()
    end
